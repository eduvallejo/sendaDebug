...
// alto[cantusExtendido.length - 3] = //la sensible 
...
// alto[cantusExtendido.length - 1] = notasMusicales[notasMusicales.indexOf(cantus[cantus.length - 1]) + randomFunction(intervaloAltoFin)];
...
function crearAlto(argument) {
	...
	for (var i = 1; i < (cantusExtendido.length - 3); i++) {//las dos ultimas estan predefinidas
		...
		//aqui se ponen las reglas del alto 
			...
			||checkForbiddenMelodicInterval(alto[i - 1], alto[i])
			// ||checkForbiddenMelodicInterval(alto[i], alto[i + 1])
